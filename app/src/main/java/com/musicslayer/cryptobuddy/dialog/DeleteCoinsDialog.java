...
public class DeleteCoinsDialog extends BaseDialog {
    ...
    public DeleteCoinsDialog(Activity activity, String coinType) {
        super(activity);
    ...
    public void updateLayout() {
        LinearLayout L = findViewById(R.id.delete_coins_dialog_checkBoxLayout);
        ...
        C[0].setText("Delete found coins from the app's database.");
        C[0].setLayoutParams(LP);

        C[1] = new CheckBox(this.activity);
        C[1].setChecked(state.get(1));
        C[1].setText("Delete custom coins from the app's database.");
